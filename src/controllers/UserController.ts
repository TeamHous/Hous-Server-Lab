...
const createUser = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    if (!result) errorGenerator({ statusCode: 409 }); //return res.status(statusCode.CONFLICT).send(util.fail(statusCode.CONFLICT, message.DUPLICATED));
...
const signInUser = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    if (!result) errorGenerator({ statusCode: 404 }); //return res.status(statusCode.NOT_FOUND).send(util.fail(statusCode.NOT_FOUND, message.NOT_FOUND));
